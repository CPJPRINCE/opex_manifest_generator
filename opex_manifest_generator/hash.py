...
class HashGenerator():
    ...
    def hash_generator_multithread(self, files_list: Iterable[str], max_workers: Optional[int] = 1, pax_flag: Optional[bool] = False) -> Dict[str, str]:
        try:
                ...
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    for folder in pax_folder_list:
                        ...
                        for root,_,files in os.walk(folder):
                            for file in files:
                                future_to_hash = {executor.submit(self.hash_generator, os.path.join(root, file)): os.path.join(root, file) if os.path.isfile(os.path.join(root, file)) else None}
                                hash = process_future(future_to_hash)
                                pax_results.append({'path': os.path.relpath(os.path.join(root,file),folder).replace('\\','/').replace('\\','/'), 'value': hash, 'type': self.algorithm})
                        hash_results.update({folder: pax_results})
            ...
            logger.warning('Multi Hash generation interrupted by user.')
            raise
        except FileNotFoundError as e:
            logger.exception(f'File Not Found generating Hash: {e}')
            raise
